...
from jaiai.etc.errors import ModelingError
    ...
    def __init__(
        ...
        def collate_fn(batch):
            ...
            if type(dataset).__name__ == "_StreamingDataSet":  # noqa: SIM108
                _tensor_names = dataset.tensor_names  # type: ignore
            else:
            ...
            if len(batch[0]) != len(_tensor_names):  # type: ignore
                raise ModelingError(
                    f"Dataset contains {len(batch[0])} tensors while there are {len(_tensor_names)} tensor names"  # type: ignore
            ...
            ret = {name: [] for name in tensor_names}  # type: ignore
            for example in batch:
                for name, tensor in zip(_tensor_names, example, strict=False):  # type: ignore
            ...
            for key in ret:
                ret[key] = torch.stack(ret[key])  # type: ignore
