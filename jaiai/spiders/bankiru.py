...
class OnePageSpider(CrawlSpider):
    ...
    rules = (
        Rule(
            LinkExtractor(restrict_xpaths="//div[@class = 'responses__item__message']/a"),
            callback="parse_item",
            follow=True,
        ),
    )
    ...
    def parse_item(self, response):
        ...
        date_published = (
            ...
            or response.xpath("//time[@itemprop='datePublished']/@datetime").get()
            or response.xpath("//time[@data-test='responses-datetime']/@datetime").get()
            or response.xpath("//time[@pubdate]/@datetime").get()
        )
        if date_published:
...
class AllPagesInvestmentSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for review in reviews:
            ...
            review_text_parts = review.xpath(
                ".//div[@class='responses__item__message markup-inside-small markup-inside-small--bullet' and @data-full]//text()"
            ).getall()
            ...
            if not item.get("review"):
                ...
                if preview_parts:
                    ...
                    preview_text = " ".join([part.strip() for part in preview_parts if part.strip() and "Читать далее" not in part])
            ...
            if item.get("title") or item.get("review"):
                ...
                self.logger.info(
                    f"📝 Отзыв {self.current_count}/{self.max_count or '∞'}: {item.get('title', 'Без заголовка')[:50]}..."
                )
